...
const smoothSignalInput = document.getElementById('smoothSignal');
...
const SMOOTH_LP_HZ = 20; // Hz (2nd-order Butterworth display smoothing)
let smoothLp = null;

// Smooth on/off transitions: avoid spikes when enabling/disabling IIR smoothing mid-stream
let smoothMix = 0;        // 0 = raw, 1 = smoothed
let smoothTargetMix = 0;  // desired mix from checkbox
const SMOOTH_RAMP_SEC = 0.25; // ramp duration (seconds)
const SMOOTH_RAMP_STEP = 1 / (SMOOTH_RAMP_SEC * PROCESS_RATE);
let hpPrevIn  = 0;
let hpPrevOut = 0;
let lpPrevOut = 0;
...
// 2nd-order Butterworth low-pass biquad (RBJ cookbook, Q=1/sqrt(2))
function makeButterworthLowpassBiquad(fc, fs) {
  const f = Math.max(0.0001, Math.min(fc, 0.499 * fs));
...
async function startMonitoring() {
  ...
  try {
    ...
    // Display smoothing filter state
    smoothLp = makeButterworthLowpassBiquad(SMOOTH_LP_HZ, PROCESS_RATE);
    if (smoothLp && smoothLp.reset) smoothLp.reset();
    smoothMix = (smoothSignalInput && smoothSignalInput.checked) ? 1 : 0;
    smoothTargetMix = smoothMix;
...
async function start30sRecording() {
  ...
  recordSmoothSignal = smoothSignalInput ? smoothSignalInput.checked : false;
...
function finishRecordingAndExport() {
  ...
  generateCsvFromRaw(tim, sig, recordRawFrequency.slice());
...
function setFilterInputsEnabled(enabled) {
  ...
  if (smoothSignalInput) smoothSignalInput.disabled = disabled;
}
...
function processSample() {
  ...
  // Optional display smoothing (2nd-order Butterworth low-pass @ 20 Hz)
  // IMPORTANT: never reset the IIR state while running, otherwise you get a visible spike.
  // We always run the filter in the background to keep its state in sync, and just ramp
  // between raw and smoothed output when the checkbox changes.
  if (!smoothLp) smoothLp = makeButterworthLowpassBiquad(SMOOTH_LP_HZ, PROCESS_RATE);
  const smoothY = smoothLp.process(filteredSignal);

  smoothTargetMix = (smoothSignalInput && smoothSignalInput.checked) ? 1 : 0;
  if (smoothMix < smoothTargetMix) smoothMix = Math.min(smoothTargetMix, smoothMix + SMOOTH_RAMP_STEP);
  if (smoothMix > smoothTargetMix) smoothMix = Math.max(smoothTargetMix, smoothMix - SMOOTH_RAMP_STEP);

  const finalSignal = filteredSignal * (1 - smoothMix) + smoothY * smoothMix;

  // push to live buffers
  const t = audioCtx.currentTime;
  signalBuffer.push(finalSignal);
  timeBuffer.push(t);
  ...
  if (recordingActive) {
    ...
    recordSignal.push(finalSignal);
    recordTime.push(relT);
    recordRawFrequency.push(peakFrequency);
  ...
  updatePeakDetection(finalSignal, t);
}
...
function generateSnapshotImage(timeArr, signalArr, peakTimes) {
  ...
  const smoothLabel = recordSmoothSignal ? 'On' : 'Off';
  c.fillText(`Smoothing: ${smoothLabel}`, marginX, 52);
...
function generateCsvFromRaw(rawTime, rawSignal, rawFrequency) {
  if (!csvDownloadLink) return;
  if (!rawTime || !rawSignal || !rawFrequency ||
      rawTime.length < 2 ||
      rawSignal.length !== rawTime.length ||
      rawFrequency.length !== rawTime.length) {
    return;
  }
  ...
  const { signal: uniFreq } =
    resampleToUniformGrid(rawTime, rawFrequency, RECORD_DURATION_SEC, PROCESS_RATE);
  ...
  csv += `# Smoothing: ${recordSmoothSignal ? 'On' : 'off'}\n`;
  csv += 'time_s,value,raw_peak_frequency_hz\n';

  for (let i = 0; i < uniTime.length; i++) {
    const t = uniTime[i].toFixed(3);   // 0.000, 0.005, ...
    const v = uniSig[i].toFixed(4);
    const f = uniFreq[i].toFixed(5);
    csv += `${t},${v},${f}\n`;
  }
...
if (smoothSignalInput) {
  smoothSignalInput.addEventListener('change', () => {
    // Do not reset the IIR filter state (that causes a spike). Just ramp the mix.
    smoothTargetMix = smoothSignalInput.checked ? 1 : 0;
  });
}
